...
window.DeseoUploadMobile = (function () {
    ...
    function applyCapture(input) {
        ...
        // Documento, selfie y video de verificación: solo cámara en móvil
    ...
    async function handleFileChange(input, btn, nameEl) {
        ...
        if (file.type.startsWith('image/')) {
            // Docs/selfie: comprimir fuerte para que entren con el resto del formulario
            const tight = role === 'document' || role === 'selfie' || role === 'user';
            finalFile = await compressImageFile(
                file,
                tight ? 0.7 * 1024 * 1024 : MAX_IMAGE_BYTES,
                tight ? 1600 : MAX_DIMENSION
            );
            if (finalFile !== file) {
