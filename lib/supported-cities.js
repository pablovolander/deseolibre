...
function getCountryLabel(countryCode) {
    return COUNTRY_LABELS[String(countryCode || '').toUpperCase()] || countryCode || '';
}
...
function validateCountryCity(country, cityName) {
    ...
    if (!COUNTRY_LABELS[countryCode]) {
        return { ok: false, error: 'Selecciona un país (México, Colombia o Argentina)' };
    }
    ...
    if (cityCheck.city.country !== countryCode) {
        return { ok: false, error: 'La ciudad no corresponde al país seleccionado' };
    }
