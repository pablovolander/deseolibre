...
function validatePublicBodyVideoUpload({ file, bodyVideoUrl, durationSec, detectedCode, expectedCode, isVercel }) {
    const hasUrl = Boolean(bodyVideoUrl && String(bodyVideoUrl).trim());
    if (!hasUrl) {
    ...
    if (duration > MAX_VIDEO_DURATION_SEC + 0.75) {
        return {
            ok: false,
