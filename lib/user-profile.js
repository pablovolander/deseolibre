...
const { validateProfileCategory, userHasProfileCategory } = require('./content-category');
const {
...
function validateUserProfileFields(body, options = {}) {
    const requireCategory = options.requireCategory || body.require_category;
    ...
    const categoryCheck = validateProfileCategory(body.category, requireCategory);
    if (!categoryCheck.ok) {
...
function userHasCompleteProfile(user) {
    ...
    return nameOk && phoneOk && telegramOk && priceOk && userHasProfileCategory(user)
        && userHasPublicBodyVideo(user) && userHasRequiredServices(user);
}

function getProfileIncompleteMessage() {
    return 'Completa tu perfil: nombre, categoría, ciudad, zona, teléfono, tarifa, servicios y video corporal público antes de publicar.';
}
