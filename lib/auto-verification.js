...
const MIN_FACE_MATCH_SCORE = 0.45;
...
function evaluateAutoVerification(payload) {
    ...
    const hasDirectVideoUrl = Boolean(body_video_url && String(body_video_url).trim());
    if (hasDirectVideoUrl) {
        const url = String(body_video_url).trim();
        const okUrl =
        ...
        if (!okUrl) {
            return { approved: false, reason: 'URL de video inválida' };
        }
        checks.push({ ok: true });
    } else {
        checks.push(validateUploadedFile(body_video, { kind: 'video', isVercel }));
    }
    ...
    return {
        approved: true,
        method: hasDirectVideoUrl ? 'auto_rules_v1_direct_blob' : 'auto_rules_v1',
        checks_passed: [
            'country',
            'document_type',
            'id_front',
            ...
            'selfie_with_id',
            hasDirectVideoUrl ? 'body_video_direct_url' : 'body_video_present',
            'body_video_duration',
