...
/** Propaga servicios ofrecidos a todas las entradas del índice de ese usuario. */
async function refreshUserOfferedServicesInFeedIndex(userId, offeredServices, category) {
    if (!getBlobToken() || userId == null) {
        return;
    }

    const uid = String(userId);
    const ids = typeof offeredServices === 'string' ? offeredServices : JSON.stringify(offeredServices || []);
    const labels = require('./service-catalog').resolveServiceLabels(category, ids);
    const index = await readFeedIndex();
    let changed = false;
        ...
        index.byCategory[cat] = list.map((item) => {
            if (item && String(item.user_id) === uid) {
                ...
                return {
                    ...
                    offered_services_labels: labels
                };
            }
            return item;
        });
...
/** Quita todas las entradas del índice de feeds de un usuario eliminado. */
async function removeUserFromFeedIndex(userId) {
    if (!getBlobToken() || userId == null) {
        return;
    }
    ...
    Object.keys(index.byCategory || {}).forEach((cat) => {
        ...
        const filtered = list.filter((item) => !item || String(item.user_id) !== uid);
        if (filtered.length !== list.length) {
            changed = true;
            index.byCategory[cat] = filtered;
        }
    });
